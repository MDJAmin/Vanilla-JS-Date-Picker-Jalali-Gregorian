document.addEventListener("DOMContentLoaded", function () {
    ...
    let currentLanguage = "fa"; // 'fa' برای فارسی و 'en' برای انگلیسی
  
    function updateCalendar() {
      if (typeof jalaali !== 'undefined') {
        const jalaaliDate = jalaali.toJalaali(currentDate.getFullYear(), currentDate.getMonth() + 1, 1);
        const daysOfWeek = currentLanguage === "fa" ? ["ش", "ی", "د", "س", "چ", "پ", "ج"] : ["S", "M", "T", "W", "T", "F", "S"];
        ...
        if (currentLanguage === "fa") {
          ...
          calendarTitle.textContent = `${currentDate.getFullYear()} ${gregorianMonths[currentDate.getMonth()]}`;
        }
        ...
        document.querySelectorAll(".day").forEach(dayElement => {
          dayElement.addEventListener("click", function () {
            if (currentLanguage === "fa") {
              ...
              datepicker.value = `${currentDate.getFullYear()}/${currentDate.getMonth() + 1}/${dayElement.textContent}`;
            }
        ...
        if (monthSelect.value === "next") {
          ...
          const nextJalaaliDate = jalaali.toJalaali(nextDate.getFullYear(), nextDate.getMonth() + 1, 1);
          const nextJalaaliMonthDays = jalaali.jalaaliMonthLength(nextJalaaliDate.jy, nextJalaaliDate.jm);
  
          if (currentLanguage === "fa") {
            nextCalendarTitle.textContent = `${nextJalaaliDate.jy} ${jalaaliMonths[nextJalaaliDate.jm - 1]}`;
          } else {
            nextCalendarTitle.textContent = `${nextDate.getFullYear()} ${gregorianMonths[nextDate.getMonth()]}`;
          }
  
          nextCalendar.querySelector('.calendar-body').innerHTML = `
            ${daysOfWeek.map(day => `<div>${day}</div>`).join('')}
            ${Array.from({ length: nextJalaaliMonthDays }, (_, i) => `<div class="day">${i + 1}</div>`).join('')}
          `;
